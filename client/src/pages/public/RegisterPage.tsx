...
import { Home, Loader2, Mail, Lock, User, Building, Users } from 'lucide-react'
import api from '@/lib/api'
import { useAuthStore } from '@/store/authStore'
import { signInWithGoogle } from '@/lib/supabase'
import toast from 'react-hot-toast'

const OCCUPATIONS = ['Salaried (Private)', 'Salaried (Government)', 'Self-Employed', 'Business Owner', 'Student', 'Freelancer']
...
export default function RegisterPage() {
  ...
  const handleGoogleSignup = async () => {
    // Store the selected role so /auth/callback can use it
    localStorage.setItem('pendingGoogleRole', form.role)
    setLoading(true)
    try {
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col justify-center py-12 px-6 lg:px-8">
      <div className="sm:mx-auto sm:w-full sm:max-w-md">
        ...
        <p className="mt-2 text-center text-sm text-slate-600">
          ...
          <Link to="/login" className="font-medium text-primary hover:text-primary-700 transition-colors">Sign in</Link>
        </p>
      </div>
      ...
      <div className="mt-8 sm:mx-auto sm:w-full sm:max-w-md">
        <div className="bg-white py-8 px-6 shadow-xl shadow-slate-200/50 rounded-3xl sm:px-10 border border-slate-100">
          ...
          {/* Role selector — shown at top so Google knows which role */}
          <div className="mb-6">
            <p className="text-sm font-semibold text-slate-700 mb-3">I am signing up as a...</p>
            <div className="grid grid-cols-2 gap-3">
              {([
                { value: 'TENANT', label: 'Tenant', icon: Users, color: 'tenant' },
                { value: 'OWNER',  label: 'Property Owner', icon: Building, color: 'owner' },
              ] as const).map(opt => (
                <button key={opt.value} type="button" onClick={() => setForm(f => ({ ...f, role: opt.value }))}
                  className={`flex items-center justify-center gap-2 p-3.5 rounded-xl border-2 font-semibold text-sm transition-all ${
                    form.role === opt.value
                      ? opt.color === 'tenant' ? 'border-tenant bg-tenant-50 text-tenant' : 'border-owner bg-owner-50 text-owner'
          ...
          <button type="button" onClick={handleGoogleSignup} disabled={loading}
            ...
            Sign up with Google as {form.role === 'TENANT' ? 'Tenant' : 'Property Owner'}
          </button>
